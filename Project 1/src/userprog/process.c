...
tid_t process_execute(const char *file_name)
{
  ...
  char *saveptr, *t_name;
  char copy_str[256];
  strlcpy(copy_str, file_name, strlen(file_name) + 1);

  t_name = strtok_r(copy_str, " ", &saveptr);
...
start_process(void *file_name_)
{
  ...
  if (!success)
    thread_exit();
  /* Start the user process by simulating a r1eturn from an
...
bool load(const char *file_name, void (**eip)(void), void **esp)
{
  ...
  for (token = strtok_r(file_name, " ", &saveptr); 
    ...
    parsed_cmd[argc++] = token;

  file = filesys_open(parsed_cmd[0]);
  if (file == NULL)
  {
  ...
  {
    argv[i] = malloc(strlen(parsed_cmd[i]) + 1);
    memcpy(argv[i], parsed_cmd[i], strlen(parsed_cmd[i]) + 1);
  }
  argv[argc] = NULL;
